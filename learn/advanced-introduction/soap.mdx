---
title: Project SOAP
description: How Quai turns merge-mining into token buybacks
---
...
KAWPOW block producers can include up to 9 workshares per block (hard cap), with a soft target of 3 workshares per block maintained by a separate difficulty adjustment algorithm. Each algo (SHA256d, Scrypt, KAWPOW) has its own share difficulty target, preventing any single algo from dominating inclusion.
...
**Example:** A block with 1000 KAWPOW entropy plus 200 workshare entropy (150 SHA256d, 50 Scrypt) totals 1200 units. An attacker must produce 20% more work to reorg compared to a KAWPOW-only chain.
