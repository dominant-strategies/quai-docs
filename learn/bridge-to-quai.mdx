...
<div style={{ display: 'flex', gap: '12px', flexWrap: 'wrap' }}>
  <button
    onClick={async () => {
      try {
        if (!window?.ethereum) {
          alert('No injected wallet detected (e.g., MetaMask).')
          return
        }
        // Ensure the site is authorized to access the selected account
        try {
          await window.ethereum.request({ method: 'eth_requestAccounts' })
        } catch (authErr) {
          console.error(authErr)
          alert('Please authorize this site to access your wallet.')
          return
        }
        await window.ethereum.request({
          ...
          // If Base is not added, prompt to add it
          if (err?.code === 4902) {
            await window.ethereum.request({
        ...
        await window.ethereum.request({
          ...
          params: {
            ...
            options: {
              ...
              symbol: 'QUAI',
              decimals: 18,
              image: 'https://pbs.twimg.com/profile_images/1882168961061531648/fsmniyg7_400x400.jpg',
            },
          },
        })
      } catch (e) {
        console.error(e)
        alert('Failed to add token. Please connect and try again or add it manually in your wallet.')
      }
    }}
