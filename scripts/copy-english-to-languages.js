...
function addTranslationNotice(content, language) {
  // Add metadata tag at the very beginning
  content = `${language.metaTag}\n${content}`;
  
  // Check if the file has frontmatter
  if (content.includes('---\n')) {
    // Find where frontmatter ends
    const frontmatterEndMatch = content.match(/^(<!-- AUTO-GENERATED-CONTENT:START \(ENGLISH-COPY\) -->\n---\n[\s\S]*?\n---\n)/);
    if (frontmatterEndMatch) {
      // Insert notice after frontmatter
      return content.replace(frontmatterEndMatch[0], `${frontmatterEndMatch[0]}\n${language.notice}\n`);
    }
  }
  
  // If no frontmatter, insert notice after metadata tag
  return content.replace(language.metaTag + '\n', `${language.metaTag}\n${language.notice}\n\n`);
}
