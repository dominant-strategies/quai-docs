...
- Will be forward compatible with [dynamic scaling events](/learn/advanced-introduction/poem/infinite-execution-shards/dynamic-sharding)
...
| [Solidity Documentation](https://docs.soliditylang.org/en/latest/)           | The official Solidity documentation.                                 |
...
- Support for cross-chain transactions via the [`etx`](/build/smart-contracts/opcode-additions#etx) opcode.
- Support for cross-chain address validation via the [`isaddrInternal`](/build/smart-contracts/opcode-additions#isaddrinternal) opcode.

Additional opcode usage is currently supported via [inline assembly](https://docs.soliditylang.org/en/latest/assembly.html). More details on the usage of `ext` and `isaddrinternal` can be found on the [Opcode Additions page](/build/smart-contracts/opcode-additions).
...
| [Opcode Additions](/build/smart-contracts/opcode-additions)                                       | Additional opcodes added to SolidityX for cross-chain functionality. |
...
For any of the above languages to be used on Quai Network, support for Quai's additional EVM opcodes [`isaddrInternal`](/build/smart-contracts/opcode-additions#isaddrinternal) and [`etx`](/build/smart-contracts/opcode-additions#etx) must be added to their respective compilers. More details on using the Quai specific opcodes can be found on the [Opcode Additions page](/build/smart-contracts/opcode-additions). Quai Network is exploring full opcode support for these alternative languages in the future.
