...
Further information on RPC endpoints can be found in the [local network specifications](/ko/build/networks#local-network) section for **local nodes** and the [devnet specifications](/ko/build/networks#devnet) section for **remote nodes**.

The `hardhat-example` repository uses the [Quais SDK](/ko/sdk/introduction) to configure network connections using only a single RPC URL. To learn more about how the SDK configures network providers, visit the [SDK provider examples](/ko/sdk/static/provider) section.
...
<Steps>
  ...
  <Step title="Interact with the contract">
    ...
    To interact with the contract, you can configure a simple script using the [Quais SDK](/ko/sdk/static/contract). The script below configures a [Contract](/ko/sdk/content/classes/Contract) instance for the ERC20 token we deployed to `0x00735E9B2c731Fd3eCC8129a3653ACb99dF969cC` to get the name, symbol, and total supply of the token.
