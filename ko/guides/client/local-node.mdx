...
To interact with the local network instance, you can use the [Quais SDK](/ko/sdk/introduction) or [JSON RPC API](/ko/build/playground/overview). The network is accesible via specific networking ports on `localhost`.
...
You can make requests to your local network using the [JSON RPC API](/ko/build/playground/overview) and [cURL](https://curl.se/):
