...
title: SOAP 项目
description: Quai 如何通过补贴式公开市场收购协议（SOAP）将合并挖矿转化为代币回购
--------------------------------

SOAP（补贴式公开市场收购协议）将合并挖矿奖励转化为持续的 QUAI 回购。购买的 QUAI 被全部销毁（100%）。
...
SOAP 将合并挖矿转化为一种协议补贴机制。矿工不再直接收到父链的区块奖励；相反，SOAP 会把父链 coinbase 输出的 100% 路由到一个由协议控制的地址。该地址随后在市场上把父链代币兑换成 QUAI，并将买到的 QUAI 全部销毁。
...
对只专注 QUAI 的矿工来说，一切不变：他们挖 KawPow 区块并获得 QUAI。对已有 BCH/LTC/DOGE 硬件的矿工来说，SOAP 提供了一种新方式来赚 QUAI：你只需把 SHA-256 或 Scrypt 设备指向支持 SOAP 的矿池来挖 QUAI，同时合并挖到的 BCH/LTC/DOGE 会被用于回购。
...
这里是 SOAP 在协议设计层面变得有趣的地方。Quai 区块只能由 KawPow 矿工生成，其他算法不能直接创建 Quai 区块。但其他算法（BCH 的 SHA-256、LTC/DOGE 的 Scrypt，以及 KawPow 本身）可以提交 workshare，被 KawPow 区块打包，并获得 QUAI 奖励。
...
KawPow 区块生产者每个区块最多可包含 9 个 workshare（硬上限），并由单独的难度调整算法维持每块约 3 个 workshare 的软目标。每种算法（SHA-256、Scrypt、KawPow）都有独立的 share 难度目标，防止单一算法垄断打包。
...
**示例：** 一个区块自身有 1000 单位 KawPow 熵，加上 200 单位 workshare 熵（其中 150 为 SHA-256，50 为 Scrypt），总计 1200 单位。相比仅 KawPow 的链，攻击者重组需要多 20% 的工作量。
...
SOAP 通过把父链补贴路由到协议控制地址并用于回购 QUAI，从而把传统合并挖矿转变为协议补贴机制。回购的 QUAI 随后被全部销毁。这在允许外部矿工通过 workshare 赚取 QUAI 的同时，持续制造买压。

在技术上，SOAP 使用 AuxPoW 结构证明父链参与，把承诺放进 coinbase 的 scriptSig 中，以保持对标准挖矿基础设施的兼容。Quai 区块仅由 KawPow 生成（正从 ProgPoW 过渡），但来自其他算法（SHA-256、Scrypt 以及 KawPow 自身）的 workshare 可以被包含并获得 QUAI 奖励，从而让不同硬件类别共同提供安全性。
