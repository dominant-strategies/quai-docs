...
To connect to a remote node provider, use the [JsonRpcProvider](/cn/sdk/content/classes/JsonRpcProvider) or [WebSocketProvider](/cn/sdk/content/classes/WebSocketProvider) classes. This connects to a remote node using the JSON-RPC interface. **Remote node providers will never have a signer bundled into them**.
...
There are multiple ways to configure a network provider using the [Provider](/cn/sdk/content/interfaces/Provider) interface.

To connect to a browser wallet provider like Pelagus, use the [BrowserProvider](/cn/sdk/content/classes/BrowserProvider) class. Pelagus bundles a Signer into the injected provider that can be used to sign transactions.
