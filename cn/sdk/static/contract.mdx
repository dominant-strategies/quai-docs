...
In order to interact with a contract in any read-only capacity, you must "connect" to it using a [Provider](/cn/sdk/content/interfaces/Provider). You'll also need to pass the contract's deployed address and the contract's ABI.
...
If you want to execute a contract state-changing operation, you'll need to pass a [Signer](/cn/sdk/content/interfaces/Signer) to the contract.
