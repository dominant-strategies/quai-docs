...
values used and provide a [**ProgressCallback**](/cn/sdk/content/type-aliases/ProgressCallback) to receive periodic updates on the
completion status..
...
| :------ | :------ | :------ |
| `account` | [`KeystoreAccount`](/cn/sdk/content/type-aliases/KeystoreAccount) | The account to encrypt. |
| `password` | `string` \| `Uint8Array` | The password to encrypt the JSON data. |
| `options`? | [`EncryptOptions`](/cn/sdk/content/type-aliases/EncryptOptions) | The options to use when encrypting the JSON data. |
