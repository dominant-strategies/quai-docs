...
A [**Result**](/cn/sdk/content/classes/Result) is a sub-class of Array, which allows accessing any of its values either positionally by
...
Since it is possible to have a key whose name conflicts with a method on a [**Result**](/cn/sdk/content/classes/Result) or its
...
Creates a new [**Result**](/cn/sdk/content/classes/Result) for `items` with each entry also accessible by its corresponding name in
`keys`.
