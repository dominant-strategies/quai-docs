...
<CardGroup cols={1}>
	...
	<Warning>
		**NOTE** if the contract you are interacting with uses the CREATE2 opcode, quais is unable to handle the address grinding for you. You
		...
		that you intend to deploy to. For more information and examples please refer to [Sharding](/cn/learn/advanced-introduction/hierarchical-structure/sharding)
		in the docs.
	</Warning>
## Installation
...
A [Provider](/cn/sdk/content/interfaces/Provider) is the primary interface for reading data from any chain or ledger within Quai Network. With a Provider, you can query details about accounts, blocks and transactions, event logs, and much more.
...
A [Contract](/cn/sdk/content/classes/Contract) is a piece of code that has been deployed to and stored on the Quai ledger virtual machine. They do not exist on the Qi ledger.

Contract state changing operations can be executed using a [Signer](/cn/sdk/content/interfaces/Signer) and data can be from them using a [Provider](/cn/sdk/content/interfaces/Provider).
