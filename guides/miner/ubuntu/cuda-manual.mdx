...
<Note>
	...
	set to `3333`](/participate/stratum-proxy/stratum-faq#stratum-port). Note that the start command for CUDA utilizes a `-U` flag instead of
	the `-G` flag utilized for OpenCL.
</Note>
...
<Warning>
	Do not start the miner prior to confirming your node has [fully synced](/participate/node/node-faq#check-sync-status). Mining while your
