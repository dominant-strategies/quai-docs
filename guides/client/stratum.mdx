...
Now that we've built the source, we can start our proxy. We recommend using a process manager like [tmux](https://github.com/tmux/tmux/wiki) to run the proxy as a background service.
...
<Warning>
	**Running the proxy will only work for chains your node is validating state for**. Global nodes validate the state for all chains, whereas
