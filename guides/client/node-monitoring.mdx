...
<Note>
You must use the following flag on your node start command to enable metrics `--metrics.enabled`.
</Note>
...
Once both services are running, you'll need to start your Quai Network node. For information on how to start up your node, refer to the [Run a Node](/guides/client/node) guide.
...
<Steps>
	...
	<Step title='Set your Prometheus server URL'>For example if you are setting monitoring on the same machine as your node, you would use http://localhost:9090.</Step>
</Steps>
