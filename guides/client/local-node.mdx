...
<Steps>
  ...
  <Step title="Install Local Node Runner">
    To install the Local Node Runner, you'll need to clone the [quai-compose](https://github.com/dominant-strategies/quai-compose) repository and navigate to it.
...
Now that we've installed Docker Compose and navigated to the quai-compose repository, we can run the local network instance.
...
```bash
## print global logs
docker logs quai-compose-go-quai-1

## follow global logs
docker logs -f --tail 0 quai-compose-go-quai-1
```

If you'd like to access logs for a specific shard, you'll need to navigate to the `quai-compose-go-quai-1` container and print logs directly.

```bash
## navigate to go-quai container
docker exec -it quai-compose-go-quai-1 sh
