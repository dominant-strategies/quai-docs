...
<Note>
	...
	majority of users. If you're split on which kind of node to run, [read this](/participate/node/node-faq#global-v-slice-node).
</Note>
...
<Warning>
	...
	Quai Network is operating at high/maximum load, visit the [Node Overview page](/participate/node/node-overview#node-types) or the [Node
	FAQ](/participate/node/node-faq#node-hardware-specs).
</Warning>
...
If you are mining, **replace the default coinbase addresses** below with your own for the chains that you intend to mine. You can generate addresses for each shard easily with [Pelagus Wallet or Koala Wallet](/participate/use-quai/wallets).
...
Set the `SLICES` parameter to whichever slices of the network you would like to run. To learn more about [running setups other than a global node, check out the FAQ](/participate/node/node-faq#configuring-slices).

### Router Configuration

In order to connect directly to peers in the network, you'll need to make sure the [peering ports](/participate/node/node-overview#networking-and-conventions) on your router are forwarded or externally accessible.
...
If you're having issues with this step, make sure to check out the [FAQ router troubleshooting guide](/participate/node/node-faq#networking).
...
Now that we've built the source, we need to decide which type of node to run. As detailed in the [Node Overview](/participate/node/node-overview#node-types) page, users **can opt for either a** [**global node or slice node**](/participate/node/node-faq#global-v-slice-node) **depending on individual use case and hardware.**
...
Slice nodes run what is called a "slice" of Quai Network. A slice is a subset of the network that validates prime, one region, and one zone chain. In the codebase, a slice is identified by its [region and zone index](/participate/node/node-overview#networking-and-conventions). Region and zone indices are 0-indexed and range from 0-2.

To spin up a slice node, **you'll need to edit the `network.env` file to specify the slices you want to run**. [Slice specification](/participate/node/node-faq#configuring-slices) can be done by editing the `SLICES` variable, which is default set to run all slices in the network.
...
Checking the node logs output is the _best way_ to [_verify that your full node is running correctly_](/participate/node/node-faq#checking-nodelogs). You can also easily view node logs in your favorite IDE or text editor.
...
If your node has started correctly, it will begin syncing chain state from peers. There are a few ways to [check the progress of the sync](/participate/node/node-faq#check-sync-status).
...
You can also check the progress of your node's sync using the **client JSON RPC API**. The [quai_syncing](/build/apis/json-rpc-api#quai_syncing) API method can be used easily in the [Quai Postman Workspace](https://documenter.getpostman.com/view/19820580/2s935iv7GU) or via the command line.

To check the sync status of all of your nodes at once, you can use the script provided in the[ FAQ](/participate/node/node-faq#global-sync-status).
