...
<Tip>
  The Golden Age Testnet only consists of a single slice. This makes global and
  single slice nodes effectively equivalent for the duration of the testnet.
</Tip>
...
To run a **slice node** during on Orchard Testnet, you'll need a MacOS or Linux machine with the following specifications:

<CardGroup cols={2}>
  <Card icon="microchip" title="Fast CPU with 8+ cores" />
  <Card icon="usb-drive" title="24GB+ RAM" />
...
<Steps>
  ...
  <Step title="go-quai">
    Now that the dependencies are installed, we can clone the [go-quai repository](https://github.com/dominant-strategies/go-quai) from the root directory.
...
<Steps>
  ...
  <Step title="Slices">
    ...
    <Tip>The Orchard Testnet only supports the `[0 0]` slice.</Tip>
...
<Warning>
  The coinbase values below are set to dummy values. If you do not replace them
  with your own addresses, you will not recieve block rewards.
</Warning>
...
<AccordionGroup >
  ...
  <Accordion title="Download and Sync from Snapshot" icon="hard-drive">
    ...
    To restore your node's database from a snapshot you'll first need to **stop your node**. You can then download the latest official snapshot from [the official snapshot link](https://storage.googleapis.com/colosseum-db/goldenage_backups/quai-goldenage-backup.tgz) or from the CLI by running:
    ...
    <Tabs>
      <Tab title="Linux">
        ...
        ```bash
          ...
          tar -I 'zstd -T0' -xvf quai-snapshot.tar.zst

          # Copy db into db directory
          cp -r quai-snapshot ~/.local/share/go-quai
        ```
      </Tab>
      <Tab title="MacOS">
        To restore your database from a snapshot, use:
        ...
        ```bash
          ...
          # Expand compressed db
          tar --use-compress-program='zstd -T0' -xvf quai-snapshot.tar.zst

          # Copy db into db directory
          cp -r quai-snapshot ~/Library/Application\ Support/go-quai
        ```
      </Tab>
    </Tabs>
