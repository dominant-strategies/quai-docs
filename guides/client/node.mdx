...
<Steps>
  <Step title="Configure Mining Addresses">
    ...
    - `quai-coinbases`: Coinbase for Quai ledger
    - `qi-coinbases`: Coinbase for Qi ledger
...
To start your node, run the start command with the added `quai-coinbases`, `qi-coinbases`, and `node.slices` flags that we covered above.
...
```bash
./build/bin/go-quai start --node.slices '[0 0]' --node.quai-coinbases '0x0000000000000000000000000000000000000000' --node.qi-coinbases '0x0080000000000000000000000000000000000000' --node.miner-preference '0.5'
```
