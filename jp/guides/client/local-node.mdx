...
To interact with the local network instance, you can use the [Quais SDK](/jp/sdk/introduction) or [JSON RPC API](/jp/build/playground/overview). The network is accesible via specific networking ports on `localhost`.
...
You can make requests to your local network using the [JSON RPC API](/jp/build/playground/overview) and [cURL](https://curl.se/):
