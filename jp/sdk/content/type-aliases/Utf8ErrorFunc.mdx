...
A callback that can be used with [**toUtf8String**](/jp/sdk/content/functions/toUtf8String) to analysis or recovery from invalid UTF-8
data.
...
The value of the `badCodepoint` depends on the `reason`. See [**Utf8ErrorReason**](/jp/sdk/content/type-aliases/Utf8ErrorReason) for
details.
...
| Parameter | Type |
| :------ | :------ |
| `reason` | [`Utf8ErrorReason`](/jp/sdk/content/type-aliases/Utf8ErrorReason) |
| `offset` | `number` |
| `bytes` | `Uint8Array` |
| `output` | `number`[] |
| `badCodepoint`? | `number` |
