...
[**ContractEvent**](/jp/sdk/content/interfaces/ContractEvent) - A filter from the `contract.filters`, which will pass only the
EventPayload as a single parameter, which includes a `.signature` property that can be used to further filter the
event.

[**TopicFilter**](/jp/sdk/content/type-aliases/TopicFilter) - A filter defined using the standard Ethereum API which provides the specific
...
[**DeferredTopicFilter**](/jp/sdk/content/interfaces/DeferredTopicFilter) - A filter created by calling a
[**ContractEvent**](/jp/sdk/content/interfaces/ContractEvent) with parameters, which will create a filter for a specific event signautre
