...
Remote providers also generally provide a read-only connection to a single shard within Quai Network. In the example below, we are configuring our `JsonRpcProvider` to connect to `Cyprus1`. Querying this provider for any data not on the `Cyprus1` shard will result in an error.
