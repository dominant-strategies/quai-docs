...
<Tabs>
  <Tab title="Quai Transactions">
    <Steps>
      <Step title="Configure Wallet">
        ...
        const hdWallet = quais.QuaiHDWallet(mnemonic, provider)
        ```
      </Step>
  ...
  <Tab title="Qi Transactions">
      ...
      <Step title="Configure Wallet">
        ...
        // initialize HD wallet
        const hdWallet = quais.QiHDWallet(mnemonic, provider)

        // sync UTXO set
        await hdWallet.scan(quais.Zone.Cyprus1)
        ```

        Syncing the UTXO set will retrieve gap and change addresses, as well as scan the specified account for addresses with unspent outputs. You'll need the wallet outpoints and addresses to build a transaction.
      </Step>
      <Step title="Build Transaction">
